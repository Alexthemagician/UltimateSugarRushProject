...
const OBJECTIVES := {
	"chocolate_cake": {"label":"CHOCOLATE", "target":8, "node":"ChocolateObjective"},
	"strawberry_cake": {"label":"STRAWBERRY", "target":8, "node":"StrawberryObjective"},
	"vanilla_cake": {"label":"VANILLA", "target":8, "node":"VanillaObjective"},
	"blueberry_cake": {"label":"BLUEBERRY", "target":8, "node":"BlueberryObjective"},
}
...
func _reset_board() -> void:
	...
	_update_objectives()
	SaveSystem.save_now()
	status_label.text = "Cake Garden fully reset for testing."


func _update_status() -> void:
	status_label.text = "Merge matching ingredients into beautiful whole cakes."
...
func _on_item_merged(item_id: String) -> void:
	...
	if int(_objective_progress[item_id]) >= target:
		status_label.text = "%s cake objective complete!" % str(objective.label).capitalize()
	_complete_board_if_ready()


func _complete_board_if_ready() -> void:
	if _completion_started or bool(SaveSystem.get_value("progression", "level_2_complete", false)) or not _all_objectives_complete():
		return
	_completion_started = true
	SaveSystem.set_value("progression", "level_2_complete", true)
	SaveSystem.set_value("progression", "level_3_unlocked", true)
	SaveSystem.save_now()
